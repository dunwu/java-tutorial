...
/**
 * 向 Elastic 日志中心传输日志 Logback logstash-logback-encoder jar 包会根据 logback 中的配置，将日志数据定向传输到
 * logstash 详见 src/main/resources/logback.xml appender 配置 使用 udp 方式传输时，有丢失日志的情况（ELK-UDP）
 * 使用 tcp 方式传输时，不会丢失日志（ELK-TCP） Log4j 通过 org.apache.log4j.net.SocketAppender 发送 TCP
 * 数据，Logstash 服务器使用 log4j input 插件 接收
 *
 * @author Zhang Peng
 */
